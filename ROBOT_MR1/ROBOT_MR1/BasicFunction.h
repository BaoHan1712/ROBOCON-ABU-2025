...
int Home_Encoder_RL = 400, Home_Encoder_RR = 405, Home_Encoder_FL = 975, Home_Encoder_FR = 1700 ;
int	current_RL_Angle, current_RR_Angle, current_FL_Angle, current_FR_Angle;

void RobotRotate_free(int Speed);
//void veHome(){
...
//		tocdo = speedMax, tocdoXoay = 1.2;
	 if(R1)
		tocdo = speedMax, tocdoXoay = 0.75, td_x = 10;
	else
		tocdo = speedMin, tocdoXoay = 0.25, td_x = 4;
...
{
	ManualRotate = 1;
	Rear_Right_Wheel_Angle = 1050;
	Front_Left_Wheel_Angle = 450;
	Front_Right_Wheel_Angle = -450;
	
	if(Speed < 0)	Mor_FrontLeft_back, Mor_FrontRight_next, Mor_RearRight_next;
	else					Mor_FrontLeft_next, Mor_FrontRight_back, Mor_RearRight_back;
	Mor_FrontLeft = Mor_FrontRight = Mor_RearRight = abs(Speed);
...
vu8 Home_wheel_RL(void)
{
	...
	if(Home_Rear_Left == 1 && SS_RL == 0){
		mor_rotate_rear_left = 100; TIM4->CNT = 0; num_over_t4 = 0;return 0;
	}
...
vu8 Home_wheel_RR(void)
{
	...
	if(Home_Rear_Right == 1 && SS_RR == 0)																{	mor_rotate_rear_right = 100; TIM3->CNT = 0; num_over_t3 = 0;return 0;}
...
{
	mor_rotate_front_left_next;
	if(Home_Front_Left == 1 && SS_FL == 0)															{	mor_rotate_front_left = 100; TIM5->CNT = 0; num_over_t5 = 0;return 0;}
...
vu8 Home_wheel_FR(void)
{
	...
	if(Home_Front_Right == 1 && SS_FR == 0)																{	mor_rotate_front_right = 100; TIM2->CNT = 0; num_over_t2 = 0;return 0;}
...
{
		if (abs(ENCODER_RL() - Rear_Left_Wheel_Angle) > 10)
	{
		...
		speed_temp = (abs(ENCODER_RL() - Rear_Left_Wheel_Angle) * 1);
		if (speed_temp > 250)
			speed_temp = 250;
		if (speed_temp < 10)
			speed_temp = 10;
