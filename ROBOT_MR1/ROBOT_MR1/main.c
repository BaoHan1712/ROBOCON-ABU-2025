...
static void taskDieuKhienCoCau2(void *pvParameters)
{
...
static void taskDieuKhienCoCau(void *pvParameters)
{
	...
	while (1)
	{
		if(SHARE)	robotGamePadControl(12, 120);
		else			 robotStop(10);
...
static void taskRobotAnalytics(void *pvParameters)
{
	while (1)
	{
		...
		if(ManualRotate == 0)	robotAnalytics();
...
//	while(!Home_wheel_FR());
	...
	//xTaskCreate(taskDieuKhienCoCau2, (signed char *)"taskDieuKhienCoCau2", 256, NULL, 0, NULL);
	state_nang = 0;
	target_BT_Xoay = 507;
	...
	while (1)
	{
		...
		reset();
		lucbanlazer(lazeTruocValue);
		lucchuyenbong(received_distance);

		if(track == 0)	banxoay(20);
    if(L2)  Re_bong();
