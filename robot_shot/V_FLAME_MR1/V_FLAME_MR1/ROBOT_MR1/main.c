...
static void taskMain(void *pvParameters)
{
	...
	{			
     ...
     // robotGamePadControl();
			...
			Nang_Ha_Nong_Ban();
			Lay_bong();
			xoaytam(received_offset);
			Day_re();
			
			
			tinhLucBan(lazeTraiValue);
			tinhlucban_lidar(received_distance);
			calculate_force(force,calculated_force);
			LED_CHECK(received_offset);
			//final_result_lidar = calculated_force * (result_lidar)/10 ;
			nhabong();
  	  if(!R1 && TRIANGLE)	Re_Bong();
			if(!R1 && SQUARE)	dua_bong();
			//if(!R1 && R2&&CB_NHAN_BONG_BO_BAN==0)ban_bong_giua();
			if(!R1 && LJOY) tra_bong();
			
			bantheokhoangcach();
			//if(CB_NHAN_BONG_BO_BAN == 0)	test_gocban();
			vTaskDelay(1);
		}
		robotStop(0);
	}
