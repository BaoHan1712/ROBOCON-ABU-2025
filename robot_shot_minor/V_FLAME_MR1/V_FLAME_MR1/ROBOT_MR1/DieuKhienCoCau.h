...
//**************************He so cua lazer************************************
		...
		if (force == 73) result = 14.45, taget_BT_Xoay = degree_F+ 13;
	  else if (force == 74) result = 14.35, taget_BT_Xoay = degree_F + 13;
		else if (force == 75) result = 14.25, taget_BT_Xoay = degree_F + 10 ;
...
void xoaytam(int received_offset) {
    ...
    if (R2) {
        ...
        // Vùng chết (deadband) để tránh dao động
        if (abs(error) < OFFSET_TOLERANCE) {
            robotRotateStop();
            integral = 0; // Reset integral
        } else {
            robotRotateFree(output, 0);
        }
        
        prev_error = error;
    } else {
        // Reset các biến PID khi không điều khiển
        prev_error = 0;
        integral = 0;
    }
}
