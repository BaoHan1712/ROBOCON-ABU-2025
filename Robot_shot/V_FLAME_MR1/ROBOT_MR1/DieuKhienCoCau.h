...
//lidar
if (R2 && CB_NHAN_BONG_BO_BAN == 0) {
        if (TOUCHPAD) { 
            shooting = 1; 
            robotStop(10); 
            Ban_1 = Ban_2 = Ban_3 = calculated_force;
						Ban_1_next, Ban_2_next, Ban_3_next;
            while (TOUCHPAD) {
                vTaskDelay(100); 
            }
...
#include <math.h> 
#define CENTER 100
#define MAX_SPEED 1.55
#define MIN_SPEED 1.05
#define K 0.0555
    ...
    if (R2>50) { 
        if (received_offset < CENTER - OFFSET_TOLERANCE) {
            ...
            robotStop(0);
				...
				if (received_offset == 0)  			robotRotateFree(0,0);	
			}
		    if (received_offset < CENTER - OFFSET_TOLERANCE) {
						LED_TAM_LECH;
        } else if (received_offset > CENTER + OFFSET_TOLERANCE) {
...
#define GOC_MOC 605 //415

#define LUC_MAX1 250
#define LUC_MIN1 50

#define GOC_MAX1 650    //450
#define GOC_MIN1 550    //350

//#define LASER_HEIGHT_DIFF1 80
...
void tinhlucban_lidar(float received_distance) {
    float gocBanlidar = 0;
    float lucBanlidar = 0;
   // float corrected_distance = sqrt((received_distance * received_distance) + (LASER_HEIGHT_DIFF1 * LASER_HEIGHT_DIFF1));
    //lucban
    lucBanlidar = LUC_MOC * (received_distance / MOCCHU);
    ...
    if (lucBanlidar > LUC_MAX1) {
		...
		//gocban
    gocBanlidar = GOC_MOC + ((received_distance - MOCCHU) / MOCCHU) * (GOC_MAX1 - GOC_MIN1);
