...
def process_distances(dist_values):
    ...
    try:
        ...
        ## San ben trai
        # valid_distances = [dist for dist in distances if dist < 8.0 and dist > 4.0]
        
        #San ben phai
        valid_distances = [dist for dist in distances if dist < 10 and dist > 4.0]
        
        if not valid_distances:
            raise ValueError("Không có khoảng cách hợp lệ nhỏ hơn 8m va lon hon 4m")
        
        # Lấy giá trị lớn nhất trong các khoảng cách hợp lệ
        min_dist = max(valid_distances)

        ## San ben trai
        # return round((min_dist - 0.325) * 1000, 2)  

        # san ben phai
        return round((min_dist - 2.475) * 1000, 2)  
    except Exception as e:
        print(e)
...
class LidarThread(threading.Thread):
    ...
    def calculator_to_radar(self, s):
        ...
        pattern = r"DIST1\s3F800000\s00000000\sF4240\s2710\sB([\s0-9A-Fa-f]+)RSSI1"
        ...
        while self._running:
            try:
                ...
                if match:
                    ...
                    if abs(new_distance - self.previous_distance > 8000):
                        ...
                        self.last_5_values.append(new_distance)
