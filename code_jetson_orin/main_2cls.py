...
def visualize_detections(frame, basket_detected, backboard_detected, basket_info, backboard_info, conf, distance):
    ...
    if backboard_detected:
        x1, y1, x2, y2, id = map(int, backboard_info)
        w, h = x2 - x1, y2 - y1
        cx, cy = x1 + w // 2, y1 + h // 2
 
        # Chỉ tính offset cho bảng
        offset = calculator_offset_stm32(frame, cx, x1, y2)
        ...
        if offset is not None:
            offset_converted = offset * 2
            hypotenuse = calculate_angle(offset_converted)
            quydoi = int(hypotenuse)-500
...
while True:
    ...
    offset_2, min_distance = visualize_detections(frame, basket_detected, backboard_detected, basket_info, backboard_info, conf, 11)
