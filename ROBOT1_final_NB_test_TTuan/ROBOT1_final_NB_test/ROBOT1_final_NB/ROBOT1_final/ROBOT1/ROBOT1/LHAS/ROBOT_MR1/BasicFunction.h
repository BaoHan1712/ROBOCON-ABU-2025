...
int Home_Encoder_0h = 415, Home_Encoder_4h = 410, Home_Encoder_8h = 400;
...
//	int lJoy_LR = (LJOY_LR >= 5 && LJOY_LR <= 250) ? 1 : 0;
	...
	if (!UP && !DOWN && !RIGHT && !LEFT && rJoy_LR && !L1&&!R2)
	{				  // Khong dieu khien
...
//		tocdo = speedMax, tocdoXoay = 1.2;
	...
	if (!UP && !DOWN && !RIGHT && !LEFT && rJoy_LR  && robotIsRun() && !L1&&!R2
		)
		robotStop(0);
