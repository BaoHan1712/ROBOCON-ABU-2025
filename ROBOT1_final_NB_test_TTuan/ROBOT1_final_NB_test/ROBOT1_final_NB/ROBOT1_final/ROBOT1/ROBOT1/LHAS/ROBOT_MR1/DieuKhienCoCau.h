...
void Ban_tru_5(void){
						if(CB_NHAN_BONG_BO_BAN == 0)
						{
							...
							Ban_1 = final_force + 15  ;
							Ban_2 = final_force - 15;
							Ban_3 = final_force - 15; // power_shot , testluc
							Ban_1_next, Ban_2_next, Ban_3_next;
							}
...
//				
//						{
//							lazeTruocValue_Ban = final_force - 10 ;
//							if (lazeTruocValue_Ban < 590) {
//							Ban_1 = final_force + 30  ;
//							Ban_2 = final_force - 30;
//							Ban_3 = final_force -30; // power_shot , testluc
...
//	}			
	...
	void Ban_tru_10(void){
						...
						{
							lazeTruocValue_Ban = distanceRobotToBackboard ;
...
void Ban_cong_5(void){
						if(CB_NHAN_BONG_BO_BAN == 0)
						{
							...
							Ban_1 = final_force  ;
							Ban_2 = final_force ;
							Ban_3 = final_force ; // power_shot , testluc
							Ban_1_next, Ban_2_next, Ban_3_next;
							}
...
void ADCValue_Control(void){
	...
	//-------------------------------------------
	lazeTruocValue = (int)(10.02023 * cam_bien_laze_truoc + 183.290118);
...
//	}
	...
	{
		bientronangluaValue = (bientronanglua_SUM / 50) / 5;
...
void SetLucBan(int _power_shot){
	if(CB_NHAN_BONG_BO_BAN == 0)
	{
		Ban_1 = _power_shot + truhaolucban + (100 - _power_shot * 0.4);
		Ban_2 = _power_shot + truhaolucban - (100 - _power_shot * 0.4);
		Ban_3 = _power_shot + truhaolucban - (100 - _power_shot * 0.4);
		Ban_1_next, Ban_2_next, Ban_3_next;
	}
}		
...
// Hàm tính l?c t? giá tr? laser
double tinhLuc(double laser) {
	  return (-1.606e-6) * laser * laser + 0.0418 * laser - 14.78;
}
//=========== Ham quet tu dong====================
void xoay_lay_khoang_cach(){
    int C=0, i, MinTrai = 70, MaxPhai = 190;
	
		
		xoayNongTuDo = 1;
	
//    if(!R1 && !L1 && TRIANGLE && !X && !SQUARE && !O){
//        while (!R1 && !L1 && TRIANGLE && !X && !SQUARE && !O) {
//            vTaskDelay(1);        
//        }
       // quayGoc3Banh(900,900,900);
        ...
        CamRotate = 180;
        CamRotate_back;
        while (abs(bientronangluaValue - MinTrai) > 1) {
            vTaskDelay(1);
        }

//        while(CB_NHAN_BONG_BO_BAN)
//        {
//         vTaskDelay(1);
//         if(OPTIONS){
//             break;
...
//        }
        ...
        CamRotate_next;
        C=0;
        while (abs(bientronangluaValue - MaxPhai) > 1) {
            vTaskDelay(1);
					
            if(lazeTruocValue < 1000) {
                goto QuetTrai;
            }
            
            if(lazeTruocValue > 3000 && lazeTruocValue < 8000) {
                if(C==0) C = bientronangluaValue;
                
                if(bientronangluaValue - MinTrai > 0 && bientronangluaValue - MinTrai < 100)
                    addElement(bientronangluaValue - MinTrai, lazeTruocValue);                
						}
            
            if(C > 0 && abs(bientronangluaValue - C) > 3 ) break;            
				}
    	...
    	if (calculate_triangle((double)canh_trai, (double)canh_phai, &distanceRobotToBackboard, &TamRo) != 0) {
            CamRotate = 2;
        ...
        while (abs(bientronangluaValue - TamRo) > 1 && abs(bientronangluaValue - MinTrai) > 2) {
            if(bientronangluaValue > TamRo) CamRotate_back;
            else {
							CamRotate_next;
							CamRotate = 15;
            }
        ...
        CamRotate = 2;
				xoayNongTuDo = 0;
				target_BT_Xoay = TamRo;
				

       final_force = tinhLuc(distanceRobotToBackboard);
			 SetLucBan(final_force);
