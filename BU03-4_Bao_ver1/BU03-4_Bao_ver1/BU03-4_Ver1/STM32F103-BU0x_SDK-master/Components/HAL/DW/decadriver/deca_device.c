...
* no return value
* 
* @brief  用于读取PDOA结果，它是Ipatov和STS POA之间的相位差(在PDOA模式1中)
* 或两个STS poa (PDOA模式3)，视PDOA的运作模式而定。(POA -到达阶段)
*
* 注:要转换成度数:float pdoa_deg = ((float)pdoa / (1 << 11)) * 180 / M_PI
*
* 输入参数
*
* 输出参数- PDOA结果(以[1:-11]弧度单位签署)
*/
int16_t dwt_readpdoa(void)
{
    ...
    if (pdoa & B12_SIGN_EXTEND_TEST) pdoa |= B12_SIGN_EXTEND_MASK; //sign extend
		float pdoa_deg = ((float)pdoa / (1 << 11)) * 180 / 3.1415926535897;
    return pdoa_deg;
}
