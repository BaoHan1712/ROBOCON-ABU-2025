...
#define Mor_RearLeft_next 										DATA_SPEED[6]=1
#define Mor_RearLeft_back 										DATA_SPEED[6]=0
...
#define mor_rotate_front_right_next						DATA_SPEED[18]=0
#define mor_rotate_front_right_back						DATA_SPEED[18]=1
...
#define Ban_1_next				 				DATA_SPEED[46]=1
#define Ban_1_back								DATA_SPEED[46]=0
...
#define  CB_NHAN_BONG_BO_BAN										GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_4)// -- OK
...
#define  Home_Rear_Left											GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_15) // -- OK
#define  Home_Rear_Right										GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_2)
#define  Home_Front_Left										GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_14)
#define  Home_Front_Right										GPIO_ReadInputDataBit(GPIOE,GPIO_Pin_0)
//================================INPUT==================================//
...
#define Encoder_Front_Left	(vs32)((num_over_t5<<16) | TIM_GetCounter(TIM5))/9
#define Encoder_Front_Right	(vs32)((num_over_t2<<16) | TIM_GetCounter(TIM2))/9
#define Encoder_Rear_Left		(vs32)((num_over_t4<<16) | TIM_GetCounter(TIM4))/9
#define Encoder_Rear_Right	(vs32)((num_over_t3<<16) | TIM_GetCounter(TIM3))/9
...
void ProcessReceivedData(void)
{
    ...
    vu8 offset = 0;
    vu16 distance = 0;
    vs16 position = 0;  // D? li?u có d?u (signed 16-bit)

    // Ki?m tra header byte (0x02)
    if(RX_USART6[0] == 0x02)
    {
        // L?y d? li?u offset (1 byte)
        offset = RX_USART6[1];

        // L?y d? li?u distance (2 byte, unsigned)
        distance = (RX_USART6[2] << 8) | RX_USART6[3];

        // L?y d? li?u position (2 byte, signed)
        position = (RX_USART6[4] << 8) | RX_USART6[5];

        // Gi?i mã position có d?u
        if (position & 0x8000) {  // Ki?m tra bit d?u
            position -= 0x10000;
        }

        // Tính checksum
        calc_checksum = (RX_USART6[0] + RX_USART6[1] + RX_USART6[2] + RX_USART6[3] + 
                         RX_USART6[4] + RX_USART6[5]) % 256;

        // Ki?m tra checksum và end byte
        if(calc_checksum == RX_USART6[6] && RX_USART6[7] == 0x03)
        {
		...
		char*camera="11111111111111111111110111111111";
							...
							switch (_so_dong) {
									...
									case 2:
										...
										break;
									case 3:
										HMI_DMI("final_CAM:",final_force,3);
										break;
									case 4:
										HMI_DMI("Luc Ban_CAM: ",force_F,4);
										break;
									case 5:
										HMI_DMI("GT_Laze ",lazeTraiValue,5);
										break;
									case 6:
...
//										}
									...
									case 11:
										...
										break;
									case 12:
										HMI_DMI("distance:",received_distance,12);  						
										break;
									case 13:
										HMI_DMI("Final_Lazer:",final_force2,13);   
										break;
									case 14:
										HMI_DMI("Luc Ban Lazer",force_F2,14);  
										break;		
									case 15:
										HMI_DMI("direction:",rx_position,15); 
										break;	
								  case 16:
