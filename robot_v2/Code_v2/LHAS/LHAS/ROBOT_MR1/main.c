...
static void taskDieuKhienCoCau(void *pvParameters)
{
	...
	while (1)
	{
		if(SHARE)	{	
			...
			if(TRIANGLE)	autorun(received_offset,received_distance);
			else 			robotGamePadControl(12,80);
		}
...
//	while(!Home_wheel_RL());
	...
	while (1) {
		if(SHARE) {
			// Xử lý tính toán lực bắn
			lucbanlazer2(lazeTraiValue);
			lucbanlazer(received_distance);
