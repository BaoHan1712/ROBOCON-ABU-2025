...
float fHeSoChinhHuong = 0.1, f05 = 0.5, f1 = 1.0, f2 = 2.0, f4 = 4.0, f5 = 5.0, f6 = 6.0, f10 = 10.0, f20 = 20.0, f30 = 30.0, f60 = 60.0, f120 = 120.0, f180 = 180.0;
...
// �?nh nghia c�c gi� tr? joystick (gi? d?nh)
...
float _robotRun0h=0, _robotRun2h=0, _robotRun4h=0, _robotRun6h=0, _robotRun8h=0, _robotRun10h=0;
...
//================================== Tinh tam suat khoang cach va goc ban ==============================


//========================================================================================== 

void setMotor(int speed_0h, int speed_2h, int speed_4h, int speed_6h, int speed_8h, int speed_10h)
{
	...
	if(!speed_10h) mor_10h = motorLock;
 	else if(speed_10h > 0)
 	{
	 	mor_10h = speed_10h;
		mor_10h_next;
...
int robotIsStop()
{
    return (mor_0h < 3 && mor_2h < 3 && mor_4h < 3 && mor_6h < 3 && mor_8h < 3 && mor_10h < 3);
}
//------------------------------------------------------------------------------
int robotIsRun()
{
...
{
	if(T == 0) 			return 0;
	else 	if(T < 0) return (T*0.05 - 0.1)/_robotCurrentSpeed; // tuy thuoc robot ma co the he so toi thieu la 0.5, 1, 2...
				else 			return (T*0.05 + 0.1)/_robotCurrentSpeed;
}
//------------------------------------------------------------------------------
...
void calculateMotor(float rotate)
{
		...
		{
			_robotRun0h = _robotRun2h = _robotRun4h = _robotRun6h = _robotRun8h = _robotRun10h = 0;
		}
		else // khi robot vua chay vua quay
	{
		angle = (float)(_robotAngle - (_robotIMUAngle - _robotIMUInit) - _robotRotate * f10) / f10;
			...
			_robotRun4h		= sin(radian * (angle + f60));
			
			_robotRun6h 	= 	-sin(radian *  (angle + f180)) ;
			_robotRun8h 	= -sin(radian * (angle + f120));
			_robotRun10h 	= -sin(radian * (angle + f60)) ;
...
void robotAnalytics(void)//-------- Dat trong Interup Timer ---------
{
    ...
    if(_robotChange) setMotor(roundF(_robotCurrentSpeed * _robotRun0h), roundF(_robotCurrentSpeed * _robotRun2h), roundF(_robotCurrentSpeed * _robotRun4h), roundF(_robotCurrentSpeed * _robotRun6h), roundF(_robotCurrentSpeed * _robotRun8h), roundF(_robotCurrentSpeed * _robotRun10h));
}
