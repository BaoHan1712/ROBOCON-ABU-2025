...
float result;  // Kh?i t?o giá tr? m?c d?nh
int final_result;
...
//		tocdo = 20+GP_BTN[6]/1.5, tocdoXoay = 0.1 + GP_BTN[6]/500;
     ...
     if(R1) tocdo = 120, tocdoXoay = 0.9;
		 else tocdo = 50, tocdoXoay = 0.5;
    ...
    if(!UP && !DOWN && !RIGHT && !LEFT && rJoy_LR  && robotIsRun()) robotStop(0);   
...
void co_cau_phong_thu(void)
{


    if (TRIANGLE) 			  
		{
		toc = 130, Mor_CamRotate = toc, Mor_CamRotate_back;
		vTaskDelay(700);
		toc = 7;
		Mor_CamRotate = toc, Mor_CamRotate_back;
		vTaskDelay(1900);
		toc = 40;
		Mor_CamRotate = toc, Mor_CamRotate_back;}
		else if (ENCODER_XOAY() >= 1945) Mor_CamRotate = 2;	
	
//  	else if (ENCODER_XOAY() >= 1400 && ENCODER_XOAY() < 1950) toc = 7, Mor_CamRotate = toc, Mor_CamRotate_back;
//		else if (ENCODER_XOAY() >= 1300)                          toc = 10,Mor_CamRotate = toc, Mor_CamRotate_back;
//		else if (ENCODER_XOAY() >= 1200)                          toc = 15,Mor_CamRotate = toc, Mor_CamRotate_back;
//    else if (ENCODER_XOAY() >= 1100)                          toc = 20,Mor_CamRotate = toc, Mor_CamRotate_back;
//		else if (ENCODER_XOAY() >= 1000)                          toc = 25,Mor_CamRotate = toc, Mor_CamRotate_back;
//		


    if (X) {
        Mor_CamRotate = 30, Mor_CamRotate_next;
        while (CB_HOME_BAN_XOAY == 1) vTaskDelay(1);
        Mor_CamRotate = 2;
        RESET_ENCODER_XOAY();
    }
...
{
//	
// if(L2&&TOUCHPAD&&CB_NHAN_BONG_BO_BAN == 0){
//										shooting = 1;
//										robotStop(10);
...
//		}
//  if(CB_NHAN_BONG_BO_BAN== 0 && shooting == 1)
//	{
//	
...
//const float maxSpeed = 1.5; // T?c d? t?i da
//const float center = 100; // M?c tâm	
//	if (R2){
//    if (received_offset >= 98 && received_offset <= 102) {
//        tocdoXoaytrack = 0; 
//        robotRotateFree(tocdoXoaytrack, 0);
//    } else if (received_offset < 98) {
//        // Tính toán tocdoXoaytrack khi received_offset < 98
//        tocdoXoaytrack = maxSpeed * (1 - (98 - received_offset) / 98); // Tang t?c d? khi xa tâm
//        if (tocdoXoaytrack < 0) tocdoXoaytrack = 0; // Ð?m b?o t?c d? không âm
//        robotRotateFree(-tocdoXoaytrack, 0); // Xoay thu?n
//    } else if (received_offset > 102) {
//        // Tính toán tocdoXoaytrack khi received_offset > 102
//        tocdoXoaytrack = maxSpeed * (1 - (received_offset - 102) / 98); // Tang t?c d? khi xa tâm
//        if (tocdoXoaytrack < 0) tocdoXoaytrack = 0; // Ð?m b?o t?c d? không âm
...
//        }
//    }
//    last_state_kep = TOUCHPAD; // C?p nh?t tr?ng thái tru?c dó
//}
