...
{
    // Ki?m tra c? ng?t truy?n hoàn t?t
    if(DMA_GetITStatus(DMA2_Stream1, DMA_IT_TCIF1))
    {
        ...
        DMA_SetCurrDataCounter(DMA2_Stream1, 4);  // Gi? s? b?n dùng 6 byte thì d? 6
...
void DMA1_Stream2_IRQHandler(void)
{
    if(DMA_GetITStatus(DMA1_Stream2, DMA_IT_TCIF2))
    {
        ...
        // Vô hiệu hóa DMA để cấu hình lại
        DMA_Cmd(DMA1_Stream2, DISABLE);
        
        // Reset lại con trỏ và kích thước buffer
        DMA_SetCurrDataCounter(DMA1_Stream2, 4);
        
        // Kích hoạt lại DMA để nhận gói tiếp theo
