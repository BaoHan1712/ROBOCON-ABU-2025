...
static void taskDieuKhienCoCau(void *pvParameters)
{
	...
	while (1)
	{
		...
		if(SHARE)		robotGamePadControl(30, 75);
		else							robotStop(20);
...
//	if(L1)	vuotQuaRobot();//duoiTheoRobot();			
		...
		if(LJOY)					Chay_giua_san();//duoiTheoRobot();	
		...
		if	(bientronangbongValue <=200) 
		{		
				nuot_bong_tren();
				Chuyen_Bong();
				if(L1 && TOUCHPAD)    Chuyen_laser(lazeTruocValue_CHUYENLASER);
		
				if(L1&&TRIANGLE) 			Luc_co_dinh();	
				if(L1 && SQUARE )			Luc_co_dinh_1(); 
				if(L1 && X )					Luc_co_dinh_2(); 
				if(L1 && O )					Luc_co_dinh_3();			
		...
		{		    
			if(L2)  Re_bong();
		//	dis_DC(lazeTruocValue);
			nhan_l1(lazeTruocValue);
			nuot_bong_duoi();			
		}
		Kich_Ban();
		Nang_ha();
