...
int Home_Encoder_0h = 410, Home_Encoder_4h = 460, Home_Encoder_8h = 1512;
...
//	int lJoy_LR = (LJOY_LR >= 5 && LJOY_LR <= 250) ? 1 : 0;
	...
	if (!UP && !DOWN && !RIGHT && !LEFT && rJoy_LR && !L1)
	{				  // Khong dieu khien
...
//		tocdo = speedMax, tocdoXoay = 1.2;
	...
	if (!UP && !DOWN && !RIGHT && !LEFT && rJoy_LR  && robotIsRun() && !L1)
		robotStop(0);
...
vu8 Home_wheel_0h_Out(void)
{
	...
	else										{	mor_rotate_0h = 150; return 0;}
...
vu8 Home_wheel_0h(void)
{
	...
	if(Home_0h == 1 && SS_RL == 0)																			{	mor_rotate_0h = 150; TIM3->CNT = 0; num_over_t3 = 0;return 0;}
